...
async def add_event(name, type_, description, dat, img, tags, web_source):
    ...
    try:
        cursor.execute(
            "INSERT INTO events(name, locate, description, e_date, image, tags, web_source) VALUES (%s, %s, %s, %s, %s, %s, %s)",
            [name, type_, description, dat, img, tags, web_source])
        connection.commit()
...
async def extract_events():
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, name, description, e_date, image, web_source, locate FROM events WHERE e_date >= (%s)",
            [date_now])
